...
public class UpsideDownPyramidStructure extends AbstractAerialHellStructure
{
    ...
    public UpsideDownPyramidStructure(Structure.StructureSettings config, Holder<StructureTemplatePool> startPool, Optional<ResourceLocation> startJigsawName, int size, HeightProvider startHeight, Optional<Heightmap.Types> projectStartToHeightmap, int maxDistanceFromCenter)
    {
        super(config, startPool, startJigsawName, size, startHeight, projectStartToHeightmap, maxDistanceFromCenter, List.of());
    }

    @Override public StructureType<?> type() {return AerialHellStructures.UPSIDE_DOWN_PYRAMID;}
    ...
    @Override protected boolean isStructureChunk(Structure.GenerationContext context) {return getTerrainHeight(context) > MIN_GEN_HEIGHT;}

    @Override @Nullable protected BlockPos findStructureCenter(Structure.GenerationContext context)
    {
