...
public class GiantGanodermaApplanatumFeature extends Feature<NoneFeatureConfiguration> implements DungeonSensitiveFeatureCheck
{
    public GiantGanodermaApplanatumFeature(Codec<NoneFeatureConfiguration> codec) {super(codec);}

    @Override public List<ResourceKey<ConfiguredFeature<?, ?>>> getAssociatedConfiguredFeatures() {return AerialHellConfiguredFeatures.Lists.GIANT_GANODERMA_APPLANATUM_LIST;}
    ...
    @Override public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context)
    {
		...
		boolean canGenerate = (
            ...
            (reader.getBlockState(pos).is(AerialHellTags.Blocks.STELLAR_STONE) || reader.getBlockState(pos).getBlock() == AerialHellBlocks.STELLAR_DIRT));
                ...
                for (int z = - c - bonus; z <= c + bonus; z++)
                {
                    ...
                    if (!this.isPosInsideEllipsis(pos, a, b, c))
                    {
                        ...
                        if (downInEll || northInEll || southInEll || westInEll || eastInEll) //if pos is at ellipsis border : place cap block
                        {
                            ...
                            {
                            	reader.setBlock(placementPos, AerialHellBlocks.GIANT_GANODERMA_APPLANATUM_BLOCK.defaultBlockState()
