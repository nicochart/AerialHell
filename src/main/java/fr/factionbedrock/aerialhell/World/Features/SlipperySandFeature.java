...
public class SlipperySandFeature extends Feature<NoneFeatureConfiguration> implements DungeonSensitiveFeatureCheck
{
    public SlipperySandFeature(Codec<NoneFeatureConfiguration> codec) {super(codec);}

    @Override public List<ResourceKey<ConfiguredFeature<?, ?>>> getAssociatedConfiguredFeatures() {return AerialHellConfiguredFeatures.Lists.SLIPPERY_SAND_LIST;}
    ...
    @Override public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context)
    {
		...
		boolean canGenerate = (
                ...
                (reader.getBlockState(blockPos).is(AerialHellTags.Blocks.STELLAR_STONE) || reader.getBlockState(blockPos).getBlock() == AerialHellBlocks.STELLAR_DIRT));
        		...
        		for(int z = -radiusZ; z <= radiusZ; z++)
                {
                    ...
                    if (isPosInsideEllipsis(pos, radiusX, radiusY, radiusZ))
                    {
                    	...
                    	if (isReplaceable(reader, placementPos))
                    	{
                    		...
                    		if (isInsideInnerEllipsis) {reader.setBlock(placementPos, AerialHellBlocks.SLIPPERY_SAND.defaultBlockState(), 0);}
                    		else if (!(rand.nextInt(3) == 0)) {reader.setBlock(placementPos, AerialHellBlocks.SLIPPERY_SAND.defaultBlockState(), 0);}
                    	}
                    }
                }
        	}
