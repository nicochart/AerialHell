...
public class AerialHellTwistingVinesFeature extends Feature<AerialHellTwistingVinesConfig>
{
    ...
    public boolean place(FeaturePlaceContext<AerialHellTwistingVinesConfig> context)
    {
        ...
        WorldGenLevel worldgenworld = context.level(); BlockPos blockpos = context.origin();
    ...
    private static boolean findFirstAirBlockAboveGround(LevelAccessor world, BlockPos.MutableBlockPos mutablePos)
    {
        do {mutablePos.move(0, -1, 0); if (world.isOutsideBuildHeight(mutablePos)) {return false;}} while(world.getBlockState(mutablePos).isAir());
        mutablePos.move(0, 1, 0);
        return true;
    }

    public static void placeWeepingVinesColumn(LevelAccessor world, RandomSource rand, BlockPos.MutableBlockPos mutablePos, int height, int minAge, int maxAge, Block headBlock, Block bodyBlock)
    {
        for(int i = 1; i <= height; ++i)
        {
            if (world.isEmptyBlock(mutablePos))
            {
                if (i == height || !world.isEmptyBlock(mutablePos.above())) {world.setBlock(mutablePos, headBlock.defaultBlockState().setValue(GrowingPlantHeadBlock.AGE, Integer.valueOf(Mth.nextInt(rand, minAge, maxAge))), 2); break;}
                world.setBlock(mutablePos, bodyBlock.defaultBlockState(), 2);
            }
            mutablePos.move(Direction.UP);
        }
    }

    private static boolean isInvalidPlacementLocation(LevelReader world, BlockPos pos, boolean needsRoof)
    {
        ...
        else
        {
            ...
            return !blockstate.is(AerialHellTags.Blocks.STELLAR_DIRT) && !blockstate.is(AerialHellBlocks.SLIPPERY_SAND) && !blockstate.is(Blocks.NETHERRACK) && !blockstate.is(Blocks.WARPED_NYLIUM) && !blockstate.is(Blocks.WARPED_WART_BLOCK);
        }
    }
}
