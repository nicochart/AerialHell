...
public class AerialHellTwistingVinesFeature extends Feature<AerialHellTwistingVinesConfig> implements DungeonSensitiveFeatureCheck
{
    ...
    public static void placeWeepingVinesColumn(WorldGenLevel level, RandomSource rand, BlockPos.MutableBlockPos mutablePos, int height, int minAge, int maxAge, BlockStateProvider headProvider, BlockStateProvider bodyProvider)
    {
        for(int i = 1; i <= height; ++i)
        {
            if (level.isEmptyBlock(mutablePos))
            {
                if (i == height || !level.isEmptyBlock(mutablePos.above())) {level.setBlock(mutablePos, headProvider.getState(level, rand, mutablePos).setValue(GrowingPlantHeadBlock.AGE, Integer.valueOf(Mth.nextInt(rand, minAge, maxAge))), 2); break;}
                level.setBlock(mutablePos, bodyProvider.getState(level, rand, mutablePos), 2);
            }
