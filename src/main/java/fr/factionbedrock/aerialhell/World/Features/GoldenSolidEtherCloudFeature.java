...
public class GoldenSolidEtherCloudFeature extends Feature<NoFeatureConfig>
{
	private int bonus = -2;
	private int basicMinSize = 4+bonus;
	private int basicMaxSize = 7+bonus;
	private int littleMinSize = 2+bonus;
            ...
            for(int z = pos.getZ() - sizeZ; z < pos.getZ() + sizeZ+1; z++)
            {
                ...
                if((x - pos.getX()) * (x - pos.getX()) + (z - pos.getZ()) * (z - pos.getZ()) < sizeX*sizeZ-1+rand.nextInt(5))
                {
                	...
                	if (previousBlock == Blocks.AIR)
                		reader.setBlockState(newPos, AerialHellBlocksAndItems.GOLDEN_SOLID_ETHER.get().getDefaultState(), 0);
                }
                else
                {
                	// ~y=0
                	if (reader.getBlockState(newPos).getBlock() != AerialHellBlocksAndItems.GOLDEN_SOLID_ETHER.get() && rand.nextDouble() > 0.8)
                	{
            ...
            for(int z = pos.getZ() - sizeZ; z < pos.getZ() + sizeZ+1; z++)
            {
                ...
                if((x - pos.getX()) * (x - pos.getX()) + (z - pos.getZ()) * (z - pos.getZ()) < sizeX*sizeZ-1+rand.nextInt(5))
                {
                	...
                	if (previousTop1Block == Blocks.AIR)
                		reader.setBlockState(newPos.up(), AerialHellBlocksAndItems.GOLDEN_SOLID_ETHER.get().getDefaultState(), 0);
                	// ~y=-1
                	Block previousBottomBlock = reader.getBlockState(newPos.down()).getBlock();
                	if (previousBottomBlock == Blocks.AIR)
                		reader.setBlockState(newPos.down(), AerialHellBlocksAndItems.GOLDEN_SOLID_ETHER.get().getDefaultState(), 0);
                }
            	else
            	{
                	// ~y=+1
                	if (reader.getBlockState(newPos.up()).getBlock() != AerialHellBlocksAndItems.GOLDEN_SOLID_ETHER.get() && rand.nextDouble() > 0.95)
                	{
                	...
                	// ~y=-1
                	if (reader.getBlockState(newPos.down()).getBlock() != AerialHellBlocksAndItems.GOLDEN_SOLID_ETHER.get() && rand.nextDouble() > 0.95)
                	{
                    	int newSizeX = littleMinSize + (int)(rand.nextDouble() * ((littleMaxSize - littleMinSize) + 1));
                        int newSizeZ = littleMinSize + (int)(rand.nextDouble() * ((littleMaxSize - littleMinSize) + 1));
