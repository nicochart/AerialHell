...
public class EffectTotemItem extends Item
{
	...
	public EffectTotemItem(Item.Properties settings)
	{
		super(settings);
		this.timer = 0;
	}
		...
		if (!world.isClientSide() && timer <= 0)
		{
			if (entity instanceof LivingEntity)
			{
				...
				if (livingEntityIn.getMainHandItem().getItem() == this || livingEntityIn.getOffhandItem().getItem() == this)
				{
					...
					{
						livingEntityIn.addEffect(new MobEffectInstance(AerialHellMobEffects.HEAD_IN_THE_CLOUDS, 1000, 0));
					}
					...
					{
						livingEntityIn.addEffect(new MobEffectInstance(AerialHellMobEffects.GOD, 1200, 0));
					}
					...
					{
						livingEntityIn.addEffect(new MobEffectInstance(AerialHellMobEffects.SHADOW_IMMUNITY, 1000, 0));
					}
				}
			}
			timer = 300;
