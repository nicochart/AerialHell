...
public class SideEffectModule extends AbilityModule
{
    ...
    public static class DamageItem extends SideEffectModule
    {
        private DamageItem(int amount)
        {
            ...
            {
                if (equipmentSlot != null && itemOwner != null) {stack.hurtAndBreak(amount, itemOwner, (entity) -> entity.broadcastBreakEvent(equipmentSlot));}
            });
        }
    ...
    public static class ShrinkUsedItem extends SideEffectModule
    {
        private ShrinkUsedItem(int amount, boolean unlessCreative)
        {
            super((stack, itemOwner, equipmentSlot, usingItemInfo, damageInfo, miningInfo) ->
            {
                ...
                {
                    if (!(unlessCreative && EntityHelper.isCreativePlayer(itemOwner)) && !EntityHelper.hasEnchantment(itemOwner, Enchantments.INFINITY_ARROWS)) {stack.shrink(amount);}
                }
            });
        }
    ...
    public static class ShrinkItem extends SideEffectModule
    {
        ...
        {
            if ((unlessCreative && EntityHelper.isCreativePlayer(itemOwner)) || EntityHelper.hasEnchantment(itemOwner, Enchantments.INFINITY_ARROWS)) {return;}
        ...
        private static int tryShrink(ItemStack stack, int amountToShrink)
        {
            ...
            int willShrinkCount = Math.min(stack.getCount(), amountToShrink);
    ...
    public static class Cooldown extends SideEffectModule
    {
        ...
        private Cooldown(ToIntFunction<LivingEntity> cooldownDuration)
        {
            super((stack, itemOwner, equipmentSlot, usingItemInfo, damageInfo, miningInfo) ->
            {
                ...
                if (itemOwner instanceof Player player && cooldown != 0) {player.getCooldowns().addCooldown(stack.getItem(), cooldown);}
            });
        ...
        public static class Builder
        {
            ...
            public Cooldown of(int cooldownDuration) {return new Cooldown((entity) -> cooldownDuration);}
            public Cooldown of(ToIntFunction<LivingEntity> cooldownDuration) {return new Cooldown(cooldownDuration);}
        }
    }
}