...
public class AerialHellSignTileEntity extends TileEntity
{
	private final ITextComponent[] text = new ITextComponent[] { StringTextComponent.EMPTY, StringTextComponent.EMPTY, StringTextComponent.EMPTY, StringTextComponent.EMPTY };
	private boolean editable = true;
	private PlayerEntity editor;
	private final IReorderingProcessor[] textBeingEdited = new IReorderingProcessor[4];
	private DyeColor textColor = DyeColor.BLACK;

	public AerialHellSignTileEntity()
	{
		super(AerialHellTileEntityTypes.SIGN.get());
	}
	...
	public boolean onActivate(PlayerEntity player)
	{
		...
		for (int var4 = 0; var4 < var3; ++var4)
		{
			...
			if (style != null && style.getClickEvent() != null)
			{
				...
				{
					player.getServer().getCommandManager().handleCommand(this.getCommandSource((ServerPlayerEntity) player), clickEvent.getValue());
				}
			}
		}
		return true;
	...
	public CommandSource getCommandSource(ServerPlayerEntity player)
	{
		...
		return new CommandSource(ICommandSource.DUMMY, Vector3d.copyCentered(this.pos), Vector2f.ZERO, (ServerWorld) this.world, 2, string, (ITextComponent) text, this.world.getServer(), player);
	}
