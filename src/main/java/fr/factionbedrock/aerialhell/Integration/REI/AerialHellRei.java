...
public class AerialHellRei implements REIClientPlugin
{
    ...
    {
        IntegrationHelper.OSCILLATING_MAP.forEach((inputs, output) ->
        {
            EntryIngredient in = IntegrationHelper.toReiIngredient(inputs);
            EntryIngredient out = EntryIngredient.of(EntryStacks.of(output));

            registry.add(new OscillatingRecipeDisplay(in, out));
        });

        IntegrationHelper.FREEZING_MAP.forEach((inputs, output) ->
        {
            EntryIngredient in = IntegrationHelper.toReiIngredient(inputs);
            EntryIngredient out = EntryIngredient.of(EntryStacks.of(output));

            registry.add(new FreezingRecipeDisplay(in, out));
        });
    }
