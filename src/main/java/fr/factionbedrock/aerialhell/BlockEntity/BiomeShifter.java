...
public interface BiomeShifter
{
    ...
    static void transformRandomBlocks(Level world, BlockPos pos, BlockState state, BiomeShifter blockEntity)
    {
        ShiftType shiftType = blockEntity.getShiftType();
        int fieldSize = blockEntity.getRealFieldSize(); RandomSource rand = world.random;
        ...
        for (int i=0; i<tryNumber; i++)
        {
            ...
            if (!areSameBlockpos(pos, blockpos) && blockEntity.isValidBiomeShifterForPos(world, blockpos, pos) && world instanceof ServerLevel serverworld)
            {
                transformed = false;
    ...
    private static void sendTransformEffect(ServerLevel serverworld, BlockPos blockpos, ShiftType type)
    {
        RandomSource rand = serverworld.random;
        ParticleOptions particle = type == ShiftType.CORRUPT ? AerialHellParticleTypes.SHADOW_LIGHT : AerialHellParticleTypes.OSCILLATOR;
        ...
        serverworld.sendParticles(particle, vecpos.x(), vecpos.y(), vecpos.z(), 5, 0.0, 0.0, 0.0, 1.0);
        serverworld.playSound(null, vecpos.x(), vecpos.y(), vecpos.z(), sound, SoundSource.BLOCKS, 0.5F, rand.nextFloat() * 0.4F + 0.8F);
    }

    default boolean isValidBiomeShifterForPos(LevelReader world, BlockPos posToShift, BlockPos shifterPos)
    {
        return world.getBlockState(shifterPos).getBlock() instanceof BiomeShifterBlock && shifterPos.distSqr(posToShift) < this.getRealFieldSize() * this.getRealFieldSize();
    }
}
