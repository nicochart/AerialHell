...
public class AerialHellChestMaterials
{
    ...
    public static final Material GHOST_BOAT_SINGLE = makeChestMaterial("ghost_boat", ChestType.SINGLE, true);
    public static final Material GHOST_BOAT_LEFT = makeChestMaterial("ghost_boat", ChestType.LEFT, true);
    public static final Material GHOST_BOAT_RIGHT = makeChestMaterial("ghost_boat", ChestType.RIGHT, true);
    ...
    {
        return makeChestMaterial(name, type, false);
    }

    private static Material makeChestMaterial(String name, ChestType type, boolean forceTranslucent)
    {
        return switch (type)
        {
            case LEFT -> new Material(Identifier.fromNamespaceAndPath(AerialHell.MODID, "entity/chest/" + name + "/" + name + "_left"), forceTranslucent);
            case RIGHT -> new Material(Identifier.fromNamespaceAndPath(AerialHell.MODID, "entity/chest/" + name + "/" + name + "_right"), forceTranslucent);
            default -> new Material(Identifier.fromNamespaceAndPath(AerialHell.MODID, "entity/chest/" + name + "/" + name), forceTranslucent);
        };
    }
}