...
public class ResonatorShardRender<T extends AbstractArrow> extends EntityRenderer<T>
{
    ...
    {
        pConsumer.vertex(pPose.pose(), (float)pX, (float)pY, (float)pZ).color(255, 255, 255, 255).uv(pU, pV).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(pPackedLight).normal(pPose.normal(), (float)pNormalX, (float)pNormalZ, (float)pNormalY).endVertex();
    }
