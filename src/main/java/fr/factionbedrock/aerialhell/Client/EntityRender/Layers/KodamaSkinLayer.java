...
public class KodamaSkinLayer<S extends KodamaRenderState> extends RenderLayer<S, KodamaModel<S>>
{
   ...
   @Override public void submit(PoseStack matrices, SubmitNodeCollector queue, int light, S renderState, float limbAngle, float limbDistance)
   {
      if (!renderState.isInvisible)
      {
         ...
         queue.submitModel(this.kodamaModel, renderState, matrices, RenderType.entityTranslucent(renderState.texture), light, LivingEntityRenderer.getOverlayCoords(renderState, 0.0F), new Color(1.0F, 1.0F, 1.0F, 1.0F).getRGB(), null, renderState.outlineColor, null);
      }
   }
}
