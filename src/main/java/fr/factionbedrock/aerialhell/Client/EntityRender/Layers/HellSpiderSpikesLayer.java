...
public class HellSpiderSpikesLayer<S extends HellSpiderRenderState, M extends SpiderModel> extends RenderLayer<S, M>
{
    ...
    private RenderType getLayer(S renderState)
    {
    ...
    @Override public void submit(PoseStack matrices, SubmitNodeCollector queue, int light, S renderState, float limbAngle, float limbDistance)
    {
        if (!renderState.isInvisible)
        {
            ...
            queue.submitModel(this.spiderSpikeModel, renderState, matrices, getLayer(renderState), light, LivingEntityRenderer.getOverlayCoords(renderState, 0.0F), new Color(1.0F, 1.0F, 1.0F, 1.0F).getRGB(), null, renderState.outlineColor, null);
        }
    }
}
