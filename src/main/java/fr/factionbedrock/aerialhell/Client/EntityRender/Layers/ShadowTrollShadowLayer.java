...
public class ShadowTrollShadowLayer extends RenderLayer<ShadowTrollRenderState, ShadowTrollModel>
{
   ...
   @Override public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int packedLight, ShadowTrollRenderState renderState, float yRot, float xRot)
   {
      if (!renderState.isInvisible)
      {
         ...
         submitNodeCollector.submitModel(this.shadowTrollModel, renderState, poseStack, RenderType.entityTranslucent(renderState.texture), packedLight, LivingEntityRenderer.getOverlayCoords(renderState, 0.0F), new Color(1.0F, 1.0F, 1.0F, 1.0F).getRGB(), null, renderState.outlineColor, null);
      }
   }
}
