...
public class VoluciteGolemHeadRender extends MobRenderer<VoluciteGolemHeadEntity, VoluciteGolemRenderState, VoluciteGolemHeadModel>
{
    ...
    @Override public void submit(VoluciteGolemRenderState renderState, PoseStack matrixStack, SubmitNodeCollector orderedRenderCommandQueue, CameraRenderState cameraRenderState)
    {
        ...
        if (vec3 != null)
        {
            ...
            matrixStack.translate(0.0F, renderState.eyeHeight, 0.0F);
    ...
    private static void renderBeam(PoseStack matrixStack, SubmitNodeCollector orderedRenderCommandQueue, Vec3 beamVector, Identifier textureLocation)
    {
        ...
        matrixStack.mulPose(Axis.YP.rotationDegrees(yRotFactor * (180F / (float)Math.PI)));
        matrixStack.mulPose(Axis.XP.rotationDegrees(xRotFactor * (180F / (float)Math.PI)));
        int r = 255, g = 255, b = 255;
        float scale = textureLocation == GUARDIAN_BEAM_LOAD ? 0.3F : 1.0F;

        orderedRenderCommandQueue.submitCustomGeometry(matrixStack, getBeamRenderType(textureLocation), (pose, consumer) ->
        {
    ...
    private static void rectangle(VertexConsumer consumer, PoseStack.Pose entry, Vec3 minCoords, Vec3 maxCoords, int r, int g, int b)
    {
    ...
    private static void vertex(VertexConsumer consumer, PoseStack.Pose entry, float x, float y, float z, int red, int green, int blue, float u, float v)
    {
        consumer.addVertex(entry, x, y, z).setColor(red, green, blue, 255).setUv(u, v).setOverlay(OverlayTexture.NO_OVERLAY).setLight(LightCoordsUtil.FULL_BRIGHT).setNormal(entry, 0.0F, 1.0F, 0.0F);
    }
}