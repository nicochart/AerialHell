...
public class Tab
{
    ...
    private final ToFloatFunc<Boolean> blitU; //blit texture start on x coordinate (u). the boolean is "isPulledOut"
    private final ToFloatFunc<Boolean> blitV; //blit texture start on x coordinate (u). the boolean is "isPulledOut"
    private final int startPageIndex;
    private final int endPageIndex;

    public Tab(String name, ToIntFunction<Boolean> width, ToIntFunction<Boolean> height, Supplier<Integer> bookLeft, Supplier<Integer> bookTop, ToIntFunction<Boolean> relativeXPos, ToIntFunction<Boolean> relativeYPos, ToFloatFunc<Boolean> blitU, ToFloatFunc<Boolean> blitV, int startPageIndex, int endPageIndex)
    {
    ...
    public void render(GuiGraphics graphics, Font font, int mouseX, int mouseY, int currentPageIndex)
    {
        ...
        graphics.blit(this.texture, xDraw, yDraw, this.blitU.applyAsFloat(isPulledOut), this.blitV.applyAsFloat(isPulledOut), tabWidth, tabHeight, this.width.applyAsInt(true), this.height.applyAsInt(true));
        ...
        if (isHovered)
        {
            graphics.renderTooltip(font, Component.translatable(this.key), mouseX, mouseY);
        }
    }
}