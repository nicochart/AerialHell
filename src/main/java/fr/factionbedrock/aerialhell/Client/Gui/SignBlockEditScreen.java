package fr.factionbedrock.aerialhell.Client.Gui;
...
public class SignBlockEditScreen extends Screen
{
	...
	protected void init()
	{
		...
		this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 120, 200, 20, DialogTexts.GUI_DONE, (buttonWidget) ->
		{
			this.finishEditing();
		}));
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta)
	{
		...
		for (m = 0; m < this.text.length; ++m)
		{
			...
			if (string2 != null)
			{
				...
				this.minecraft.fontRenderer.func_238411_a_(string2, n, (float) (m * 10 - this.text.length * 5), i, false, matrix4f,	immediate, false, 0, 15728880, false);
				if (m == this.currentRow && j >= 0 && bl2)
				{
					s = this.minecraft.fontRenderer.getStringWidth(string2.substring(0, Math.max(Math.min(j, string2.length()), 0)));
		...
		for (m = 0; m < this.text.length; ++m)
		{
			...
			if (string2 != null && m == this.currentRow && j >= 0) {
				...
				if (k != j)
				{
					t = Math.min(j, k);
					int u = Math.max(j, k);
					int v = this.minecraft.fontRenderer.getStringWidth(string2.substring(0, t)) - this.minecraft.fontRenderer.getStringWidth(string2) / 2;
					int w = this.minecraft.fontRenderer.getStringWidth(string2.substring(0, u)) - this.minecraft.fontRenderer.getStringWidth(string2) / 2;
					int x = Math.min(v, w);
					int y = Math.max(v, w);
