...
public record ParagraphFromKey(int startLineIndex, int lastLineIndex, int lineStartOffset, int lineWidth, Alignment alignment, int color, String key) implements PageElement
{
    @Override public void render(Font font, GuiGraphicsExtractor graphics, float scale, List<Line> lines, int bookLeft, int bookTop, int mouseX, int mouseY)
    {
        ...
        for (int i = 0; i < textLines.size() && currentLineIndex < this.lastLineIndex + 1; i++)
        {
            ...
            {
                case LEFT -> lines.get(currentLineIndex).startX() + xOffset;
                case CENTER -> lines.get(currentLineIndex).centerX(textLines.get(i), font, scale) + xOffset;
                case RIGHT -> lines.get(currentLineIndex).rightX(textLines.get(i), font, scale) + xOffset;
            };
