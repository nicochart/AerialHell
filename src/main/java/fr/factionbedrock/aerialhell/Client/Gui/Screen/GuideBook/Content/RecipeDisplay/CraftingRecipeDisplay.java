...
{
    private static final TextureInfo CRAFTING_GRID_TEXTURE = new TextureInfo(ResourceLocation.fromNamespaceAndPath(AerialHell.MODID, "textures/gui/guide_book/recipe/crafting_grid_display.png"), 82, 37);
    ...
    @Override public void render(Font font, GuiGraphics graphics, float scale, List<Line> lines, int bookLeft, int bookTop, int mouseX, int mouseY)
    {
        ...
        //background (2x2 grid)
        graphics.blit(CRAFTING_GRID_TEXTURE.texture(), 0, 0, CRAFTING_GRID_TEXTURE.u(), CRAFTING_GRID_TEXTURE.v(), CRAFTING_GRID_TEXTURE.width(), CRAFTING_GRID_TEXTURE.height(), CRAFTING_GRID_TEXTURE.textureWidth(), CRAFTING_GRID_TEXTURE.textureHeight());
        ...
        for (int i = 0; i < ingredients.get().size(); i++)
        {
            ...
            graphics.renderItem(item.getDefaultInstance(), itemLocalX, itemLocalY);
            ...
            {
                graphics.renderTooltip(font, item.getDefaultInstance(), mouseX, mouseY);
            }
        }
        ...
        graphics.renderItem(resultItemStack, resultLocalX, resultLocalY);
        if (resultItemStack.getCount() > 1)
        {
            String amount = String.valueOf(resultItemStack.getCount());
            graphics.drawString(font, amount, resultLocalX + 17 - font.width(amount), resultLocalY + 9, 0xFF7A5C3A, false);
        }
        ...
        if (hovered && this.displayTooltip()) {graphics.renderTooltip(font, resultItemStack, mouseX, mouseY);}
