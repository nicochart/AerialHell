...
public class AerialHellDimensionSkyRenderer
{
	...
	private static final ResourceLocation AERIAL_HELL_SUN_LOCATION = AerialHell.id("textures/environment/aerial_hell_sun.png");
	private static final ResourceLocation AERIAL_HELL_MOON_PHASES_LOCATION = AerialHell.id("textures/environment/aerial_hell_moon_phases.png");
	...
	public static boolean render(ClientLevel world, Matrix4f projectionMatrix, Matrix4f modelViewMatrix, float partialTicks, Camera camera, boolean isFoggy, Runnable setupFog)
	{
		...
		if (!isFoggy)
		{
			...
			if (fogtype != FogType.POWDER_SNOW && fogtype != FogType.LAVA && !doesMobEffectBlockSky(camera))
			{
				...
				Vec3 vec3 = world.getSkyColor(camera.getPosition(), partialTicks);
				...
				worldRenderer.skyBuffer.bind();
				worldRenderer.skyBuffer.drawWithShader(matrixstack.last().pose(), modelViewMatrix, shaderProgram);
				VertexBuffer.unbind();
	    		...
	    		*/
				...
				matrixstack.mulPose(Axis.XP.rotationDegrees(world.getTimeOfDay(partialTicks) * 360.0F));
				//sun and moon
				...
				if (starBrightness > 0.0F)
				{
					...
					starBuffer.bind();
					starBuffer.drawWithShader(matrixstack.last().pose(), modelViewMatrix, GameRenderer.getPositionShader());
					VertexBuffer.unbind();
					setupFog.run();
				}
	...
	private static MeshData renderStars(Tesselator tesselator)
	{
	...
	private static void renderScatteredStars(BufferBuilder builder, int starNumber, float bigChance, float bigSizeBonus, RandomSource rand)
	{
		for (int i = 0; i < starNumber; i++)
		{
			...
			double squaredMagnitude = Mth.lengthSquared(starVec.x, starVec.y, starVec.z);
	...
	private static void renderStarCluster(BufferBuilder builder, int starNumber, Vector3f origin, Vector3f size, float bigChance, float bigSizeBonus, RandomSource rand)
	{
		for (int i = 0; i < starNumber; i++)
		{
