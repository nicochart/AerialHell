...
public class VibrantAerialBerryBushBlock extends VegetationBlock implements BonemealableBlock
{
    ...
    protected VoxelShape getShape(BlockState state, BlockGetter blockGetter, BlockPos pos, CollisionContext collisionContext) {return SHAPE;}

    @Override public @NotNull MapCodec<? extends VibrantAerialBerryBushBlock> codec() {return CODEC;}
    ...
    public InteractionResult useItemOn(ItemStack stack, BlockState state, Level worldIn, BlockPos pos, Player player, InteractionHand handIn, BlockHitResult hit)
    {
        ...
        else if (age > 13)
        {
            ...
            worldIn.playSound(null, pos, SoundEvents.SWEET_BERRY_BUSH_PICK_BERRIES, SoundSource.BLOCKS, 1.0F, 1F + worldIn.getRandom().nextFloat() * 0.2F);
