...
{
	public ShadowGrassBlock(AbstractBlock.Properties properties)
	{
		super(properties);
	...
	public void onPlantGrow(BlockState state, IWorld world, BlockPos pos, BlockPos source)
	{
		...
		{
			world.setBlockState(pos, AerialHellBlocksAndItems.SHADOW_GRASS_BLOCK.get().getDefaultState(), 2);
		}
	}
	...
	public void grow(ServerWorld worldIn, Random rand, BlockPos pos, BlockState state)
	{
	      ...
	      BlockState blockstate = AerialHellBlocksAndItems.SHADOW_GRASS.get().getDefaultState();
		...
		else
		{
			if (worldIn.getLight(pos.up()) >= 9)
			{
				for(int i = 0; i < 4; ++i)
				{
					...
					BlockState blockstate = AerialHellBlocksAndItems.SHADOW_GRASS_BLOCK.get().getDefaultState();
					
					if (worldIn.getBlockState(blockpos).isIn(AerialHellBlocksAndItems.STELLAR_DIRT.get()) && isSnowyAndNotUnderwater(blockstate, worldIn, blockpos))
					{
