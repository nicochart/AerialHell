...
public class AerialHellMushroomBlock extends MushroomBlock
{
	...
	{
		return floor.is(AerialHellTags.Blocks.STELLAR_DIRT) || floor.is(BlockTags.MUSHROOM_GROW_BLOCK);
	}
	...
	@Override public boolean growMushroom(ServerLevel world, BlockPos pos, BlockState state, RandomSource rand)
	{
		...
		if (this == AerialHellBlocks.VERDIGRIS_AGARIC)
		{
			if (hugeShroomDirection != HugeGenerationDirections.NONE)
			{
				...
				configuredfeature = world.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolder(AerialHellConfiguredFeatures.HUGE_VERDIGRIS_AGARIC).orElse(null).value(); //TODO check
			}
