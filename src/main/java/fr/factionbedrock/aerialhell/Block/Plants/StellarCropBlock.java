...
public class StellarCropBlock extends CropBlock
{
    ...
    @Override protected ItemLike getBaseSeedId() {return this == AerialHellBlocksAndItems.STELLAR_WHEAT.get() ? AerialHellBlocksAndItems.STELLAR_WHEAT_SEEDS.get() : AerialHellBlocksAndItems.BLUE_MEANIE_SPORES.get();}
    ...
    protected static float getGrowthSpeedFromMoisture(BlockState blockstate, BlockGetter world, BlockPos pos)
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int z = -1; z <= 1; z++)
            {
                ...
                if (blockState.is(AerialHellBlocksAndItems.STELLAR_FARMLAND.get()))
                {
                    bonusMoisture = 1.0F;
