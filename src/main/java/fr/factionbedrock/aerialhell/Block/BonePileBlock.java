...
public class BonePileBlock extends SnowLayerBlock
{
    ...
    */}
    ...
    private void updateLayerNumber(Level world, BlockPos pos, int newLayerNumber, Entity entityIn)
    {
        ...
        if (!world.isClientSide()) {if (newLayerNumber < 2 || world.getRandom().nextInt(4) == 0) {entityIn.spawnAtLocation((ServerLevel) world, new ItemStack(AerialHellItems.MUD_BONE.get()));}}
        else {entityIn.playSound(AerialHellSoundEvents.BLOCK_BONE_PILE_STEP_BREAK.get(), 0.5F, 0.9F + world.getRandom().nextFloat() * 0.3F);}
    }
