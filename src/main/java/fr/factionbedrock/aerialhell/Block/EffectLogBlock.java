...
public class EffectLogBlock extends ShiftableLogBlock
{
	...
	@Override public boolean onDestroyedByPlayer(BlockState state, Level level, BlockPos pos, Player player, boolean willHarvest, FluidState fluid)
	{
		...
		if (flag && state.is(AerialHellBlocksAndItems.EYE_SHADOW_PINE_LOG.get()) && !EntityHelper.isLivingEntityShadowImmune(player) && !player.isCreative())
		{
			player.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 60, 0));
			player.addEffect(new MobEffectInstance(MobEffects.DIG_SLOWDOWN, 60, 1));
		}
		
		if (flag && state.is(AerialHellBlocksAndItems.ENCHANTED_LAPIS_ROBINIA_LOG.get()) && !player.isCreative())
		{
