...
{
	public ThornyWebBlock(BlockBehaviour.Properties settings)
	{
		super(settings);
	}
	...
	@Override public void entityInside(BlockState state, Level world, BlockPos pos, Entity entityIn, InsideBlockEffectApplier handler, boolean intersects)
	{
		...
		if (world instanceof ServerLevel serverWorld && entityIn instanceof LivingEntity && !isTotallyImmune)
		{
			if (!isEntityImmuneToDamage(entityIn)) {entityIn.hurtServer(serverWorld, AerialHellDamageTypes.getDamageSource(world, AerialHellDamageTypes.WEB_THORNS), 2.0F);}
		}
	}
}
