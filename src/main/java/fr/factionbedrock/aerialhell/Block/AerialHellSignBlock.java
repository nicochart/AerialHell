...
public class AerialHellSignBlock extends AbstractSignBlock
{
	...
	public AerialHellSignBlock(AbstractBlock.Properties builder)
	{
	...
	public void onBlockPlacedBy(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack)
	{
		...
		{
			AerialHellSignTileEntity sign = (AerialHellSignTileEntity) world.getTileEntity(pos);
			if (!world.isRemote)
			{
	...
	public BlockState getStateForPlacement(BlockItemUseContext ctx)
	{
		if (ctx.getFace() == Direction.UP)
		{
			...
			return this.getDefaultState().with(FLOOR, true).with(ROTATION, MathHelper.floor((180.0 + ctx.getPlacementYaw() * 16.0 / 360.0) + 0.5 - 12) & 15).with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER);
		}
	...
	{
		return new AerialHellSignTileEntity();
	}
