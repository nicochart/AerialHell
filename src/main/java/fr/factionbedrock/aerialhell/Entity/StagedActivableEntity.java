...
public interface StagedActivableEntity extends ActivableEntity
{
    ...
    @Override default void activableAddAdditionalSaveData(CompoundTag valueOutput) //call in addAdditionalSaveData(valueOutput)
    {
    ...
    @Override default void activableReadAdditionalSaveData(CompoundTag valueInput) //call in readAdditionalSaveData(valueInput)
    {
        ActivableEntity.super.activableReadAdditionalSaveData(valueInput);
        this.setActivating(valueInput.contains("is_activating") ? valueInput.getBoolean("is_activating") : false);
        this.setActivatingTicks(valueInput.contains("activating_ticks") ? valueInput.getInt("activating_ticks") : 0);
        if (valueInput.contains("activated_once") && valueInput.getBoolean("activated_once")) {this.setAlreadyActivatedOnce();}
    }
    /* ----------------------------------------------- */
    /* ----------------------------------------------- */
    /* ----------------------------------------------- */
    ...
    private void immobilizeWithSlowness(int duration) {if (!this.getSelf().level().isClientSide()) {this.getSelf().addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, duration, 10, true, false));}}
