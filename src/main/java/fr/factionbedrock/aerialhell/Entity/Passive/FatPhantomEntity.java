...
public class FatPhantomEntity extends Phantom implements Enemy
{
   ...
   public static boolean canSpawn(EntityType<FatPhantomEntity> type, ServerLevelAccessor world, EntitySpawnReason reason, BlockPos pos, RandomSource random)
   {
   ...
   @Override
   public boolean doHurtTarget(ServerLevel serverWorld, Entity entity)
   {
	   ...
	   return super.doHurtTarget(serverWorld, entity);
   }
   ...
   public boolean hurtServer(ServerLevel serverWorld, DamageSource source, float amount)
   {
	   ...
	   if (!source.is(DamageTypes.MAGIC))
	   {
		   ...
		   {
			   List<Player> targetable_players = serverWorld.getNearbyPlayers(TargetingConditions.forCombat().range(64.0D), FatPhantomEntity.this, FatPhantomEntity.this.getBoundingBox().inflate(16.0D, 64.0D, 16.0D));
   ...
   private void addFatPhantomParticle(int number)
   {
   		...
   		{
   			this.level().addParticle(AerialHellParticleTypes.FAT_PHANTOM_SMOKE, this.getX() + random.nextFloat() - 0.5, this.getY() + 2 * random.nextFloat(), this.getZ() + random.nextFloat() - 0.5, random.nextFloat() - 0.5, random.nextFloat() -0.5, random.nextFloat() - 0.5);
   		}
   }
   ...
   @Override protected void readAdditionalSaveData(ValueInput view)
   {
      ...
      this.setPhantomSize(view.getIntOr("size", 0));
   ...
   @Override protected void addAdditionalSaveData(ValueOutput view)
   {
      ...
      view.storeNullable("anchor_pos", BlockPos.CODEC, this.orbitPosition);
   ...
   boolean canAttack(ServerLevel world, LivingEntity target, TargetingConditions predicate) {return predicate.test(world, this, target);}
   ...
   class AttackAttackingPlayerGoal extends Goal
   {
      ...
      public boolean canUse()
      {
         ...
         {
            this.tickDelay = 60;
            ServerLevel serverWorld = getServerLevel(FatPhantomEntity.this.level());
            List<Player> list = serverWorld.getNearbyPlayers(this.attackTargeting, FatPhantomEntity.this, FatPhantomEntity.this.getBoundingBox().inflate(16.0D, 64.0D, 16.0D));
            if (!attackingPlayers.isEmpty() && !list.isEmpty())
            {
               ...
               {
                  if (FatPhantomEntity.this.canAttack(serverWorld, player, TargetingConditions.DEFAULT) && attackingPlayers.contains(player))
                  {
