...
public class MudCycleMageEntity extends AbstractBossEntity
{
	...
	public static class SummonSpectralEntitiesGoal extends SummonThreeEntitiesGoal
	{
		...
		{
			MudSpectralSoldierEntity entity = AerialHellEntities.MUD_SPECTRAL_SOLDIER.create(this.getGoalOwner().level());
		...
		{
			return AerialHellEntities.MUD_SPECTRAL_GOLEM.create(this.getGoalOwner().level());
		}

		protected MudSpectralCycleMageEntity createClone()
		{
			MudSpectralCycleMageEntity entity = AerialHellEntities.MUD_SPECTRAL_CYCLE_MAGE.create(this.getGoalOwner().level());
