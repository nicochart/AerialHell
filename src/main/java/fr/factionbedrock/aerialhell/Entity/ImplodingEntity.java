...
public interface ImplodingEntity extends BaseMobEntityInterface
{
    ...
    /* ----------------------------------------------- */
    default void implodingAddAdditionalSaveData(CompoundTag valueOutput) //call in addAdditionalSaveData(valueOutput)
    {
    ...
    default void implodingReadAdditionalSaveData(CompoundTag valueInput) //call in readAdditionalSaveData(valueInput)
    {
        this.setImploding(valueInput.contains("is_imploding") ? valueInput.getBoolean("is_imploding") : false);
        this.setImplodingCooldownTicks(valueInput.contains("imploding_cooldown_ticks") ? valueInput.getInt("imploding_cooldown_ticks") : 0);
        this.setImplodingCastTicks(valueInput.contains("imploding_cast_ticks") ? valueInput.getInt("imploding_cast_ticks") : 0);
    }
    ...
    default void onImplodingStart() //server side
    {
        ...
        {
            this.getSelf().addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, this.getImplodingCastDuration(), 10, true, false));
        }
    }
