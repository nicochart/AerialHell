...
public interface MasterPartEntity extends BaseMobEntityInterface
{
    ...
    @Nullable default SpawnGroupData finalizePartSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData spawnData) //call in finalizeSpawn(level, difficulty, reason, spawnData)
    {
    ...
    default void partAddAdditionalSaveData(CompoundTag valueOutput) //call in addAdditionalSaveData(valueOutput)
    {
    ...
    default void partReadAdditionalSaveData(CompoundTag valueInput) //call in readAdditionalSaveData(valueInput)
    {
        this.getSelf().yBodyRot = valueInput.contains("master_body_rot") ? valueInput.getFloat("master_body_rot") : 0;
        
        for (PartInfo partInfo : this.getPartInfoMap().values())
        {
            if (valueInput.contains(partInfo.getName() +"_uuid"))
            {
                this.setPartStringUUID(partInfo, valueInput.getString(partInfo.getName() +"_uuid"));
            }
        }
    }
    ...
    @Nullable default PartEntity getPartByUUID(@Nullable String stringUUID)
    {
        ...
        Entity entity = this.getLevel() instanceof ServerLevel serverLevel ? serverLevel.getEntity(UUID.fromString(stringUUID)) : null;
    ...
    default void resetSelf()
    {
        if (this.getLevel() instanceof ServerLevel serverLevel)
        {
            ...
            Entity entity = self.getType().spawn(serverLevel, this.getSelf().blockPosition(), MobSpawnType.NATURAL);
            if (entity == null) {return;}
            entity.moveTo(self.getX(), self.getY(), self.getZ(), self.getYRot(), self.getXRot());
    ...
    default PartEntity summonPart(PartInfo part)
    {
        ...
        Entity entity = part.getType().create(this.getLevel());
        if (entity instanceof PartEntity summonedPart)
        {
            ...
            summonedPart.getSelf().moveTo(this.getX() + part.getUnrotatedRelativePositionOffset().x, this.getY() + part.getUnrotatedRelativePositionOffset().y, this.getZ() + part.getUnrotatedRelativePositionOffset().z, this.getSelf().getYRot(), this.getSelf().getXRot());
