...
public interface ActivableEntity extends BaseMobEntityInterface
{
    ...
    default void activableAddAdditionalSaveData(CompoundTag valueOutput) //call in addAdditionalSaveData(valueOutput)
    {
        valueOutput.putBoolean("is_active", this.isActive());
    }

    default void activableReadAdditionalSaveData(CompoundTag valueInput) //call in readAdditionalSaveData(valueInput)
    {
        this.setActive(valueInput.contains("is_active") ? valueInput.getBoolean("is_active") : false);
    }
    /* ----------------------------------------------- */
    /* ----------------------------------------------- */
    /* ----------------------------------------------- */
