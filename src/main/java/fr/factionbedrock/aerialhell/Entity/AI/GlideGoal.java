...
public class GlideGoal extends Goal
{
    ...
    private void setGlidingMotion()
    {
        if (this.goalOwner.getVelocity().y < 0.0D)
        {
            ...
            if (this.hasBlockUnder(this.goalOwner.getBlockPos().add((int)(2.5*forward.x), (int)forward.y, (int)(2.5*forward.z)), 20)) //goal owner has block forward (1 - 2 blocks in front of him)
            {
    ...
    private void glideForward(Vec3d forward) {this.goalOwner.setVelocity(this.goalOwner.getVelocity().multiply(1.0D, 0.6D, 1.0D).add(forward.x/100, 0, forward.z/100));}
    private void slowDownMovement() {this.goalOwner.setVelocity(this.goalOwner.getVelocity().multiply(0.9D, 0.6D, 0.9D));}
    private void speedUpMovement(Vec3d direction) {this.goalOwner.setVelocity(this.goalOwner.getVelocity().multiply(1.0D, 0.5D, 1.0D).add(direction.x/70, 0, direction.z/70));}
