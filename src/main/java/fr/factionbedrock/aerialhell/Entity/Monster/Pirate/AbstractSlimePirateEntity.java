...
public abstract class AbstractSlimePirateEntity extends AbstractHumanoidMonster
{
    ...
    @Override public void remove(Entity.RemovalReason removalReason)
    {
        if (!this.level().isClientSide && !this.isBaby() && this.isDeadOrDying())
        {
            ...
            for (int l = 0; l < number; ++l)
            {
                ...
                AbstractSlimePirateEntity littlePirate = this.getDieOffspringType().create(this.level(), EntitySpawnReason.TRIGGERED);
                if (littlePirate != null)
                {
                    ...
                    littlePirate.snapTo(this.getX() + (double) x, this.getY() + 0.5D, this.getZ() + (double) z, this.random.nextFloat() * 360.0F, 0.0F);
    ...
    {
        return super.hurtServer(serverLevel, damageSource, this.isBaby() ? amount * 1.5F : amount);
    }
