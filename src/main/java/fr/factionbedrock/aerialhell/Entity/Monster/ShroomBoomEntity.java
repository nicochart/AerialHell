...
public class ShroomBoomEntity extends Creeper
{
    ...
    @Override public boolean doHurtTarget(ServerLevel serverWorld, Entity attackedEntity)
    {
        if (!this.canIgnite())
        {
            ...
            boolean isAttackSuccess = attackedEntity.hurtServer(serverWorld, damagesource, damage);
            if (isAttackSuccess)
            {
            	if (kb > 0.0F && attackedEntity instanceof LivingEntity)
                {
            		((LivingEntity)attackedEntity).knockback(kb * 0.5F, (double) Mth.sin(this.getYRot() * ((float)Math.PI / 180F)), (double)(-Mth.cos(this.getYRot() * ((float)Math.PI / 180F))), damagesource, damage);
        ...
        else {return super.doHurtTarget(serverWorld, attackedEntity);}
    }
