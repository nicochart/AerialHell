...
public class AerialHellItemAbilities
{
    ...
    private static final ActionModule CURSED_TOOL_INTERACTION = ActionModule.create((stack, itemOwner, equipmentSlot, usingItemInfo, damageInfo, miningInfo) ->
    {
        ...
        livingOther.addEffect(new MobEffectInstance(AerialHellMobEffects.VULNERABILITY.get(), 40, otherIsLight ? 1 : 0));
    });
    ...
    private static final ActionModule IGNITE_OTHER = ActionModule.onOther((otherEntity) -> otherEntity.setSecondsOnFire(5));
    ...
    private static final ActionModule.Sound GENERIC_EXPLODE_SOUND = new ActionModule.Sound((itemOwner) -> new PlaySoundHelper(SoundEvents.GENERIC_EXPLODE, 1.0F, 0.5F + itemOwner.getRandom().nextFloat()));
    ...
    private static final ActionModule INVOKE_CHAINED_GOD = ActionModule.create((stack, itemOwner, equipmentSlot, usingItemInfo, damageInfo, miningInfo) -> {
        ChainedGodEntity god = AerialHellEntities.CHAINED_GOD.get().create(itemOwner.level());
        if (god != null)
        {
...
//            .build();
    ...
    public static final ItemAbility MINE_STONE_FAST = ItemAbility.builder()
            ...
            .addOnMiningModules(ModuleList.builder()
                    .addActions(MULTIPLY_MINING_SPEED.by((itemOwner,  state) -> state.getBlock() == AerialHellBlocksAndItems.STELLAR_STONE.get() || state.getBlock() == AerialHellBlocksAndItems.STELLAR_COBBLESTONE.get() || state.getBlock() == AerialHellBlocksAndItems.SHADOW_STONE.get() ? 2.0F : 1.0F))
                    .build()
            ).build();
